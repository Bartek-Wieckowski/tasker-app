...
--     ADD CONSTRAINT "global_todos_created_by_fkey" FOREIGN KEY ("created_by") REFERENCES "public"."db_users"("id") ON DELETE CASCADE;
...
-- CREATE INDEX "idx_global_todos_created_by" ON "public"."global_todos" USING "btree" ("created_by");
-- CREATE INDEX "idx_global_todos_is_public" ON "public"."global_todos" USING "btree" ("is_public");
...
--     FOR SELECT USING (is_public = true OR auth.uid() = created_by OR auth.uid() = ANY(assigned_to));
...
--     FOR UPDATE USING (auth.uid() = created_by OR auth.uid() = ANY(assigned_to));
